...
class Simulation:
    ...
    def sim_cruise(self, lane, i):
        ...
        if x == driver.speed + driver.safe_follow:
            ...
            driver.total_energy += (driver.speed)^2 # arbitrary units depending only on speed
            ...
            driver.total_energy += (x - driver.safe_follow)^2
        else:
            driver.desire = LANE_CHANGE
            self.road.set(lane, i + 1, driver) #Car moves forward by just 1 spot
            driver.total_energy += 1
        self.road.set(lane, i, EMPTY)
    ...
    def average_energy_used(self):
        ...
        for i in self.data:
            if (i[2]):  # if human
                hum_total_energy+=i[3]
                num_human+=1 
            else:
                sd_total_energy+=i[3]
                num_sd+=1
        return [hum_total_energy/num_human,sd_total_energy/num_sd]
